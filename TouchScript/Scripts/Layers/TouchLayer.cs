...
namespace TouchScript.Layers
{
    ...
    /// <para><see cref="LayerManager"/> keeps a sorted list of all layers in <see cref="LayerManager.Layers"/> which it queries when a new pointer appears. It's a layer's job to return <see cref="HitResult.Hit"/> if this pointer hits an object. Layers can even be used to "hit" objects outside of Unity's 3d world, for example <b>Scaleform</b> integration is implemented this way.</para>
    /// <para>Layers can be configured in a scene using <see cref="TouchManager"/> or from code using <see cref="TouchManager"/> API.</para>
